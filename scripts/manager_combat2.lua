...
function addNPC(sClass, nodeNPC, sName)
	...
	for _,v in ipairs(StringManager.split(sCondImmune, ",;\r\n", true)) do
	...
	for _,v in pairs(DB.getChildren(nodeEntry, "bonusactions")) do
		CombatManager2.parseNPCPower(rActor, v, aEffects);
	end
...
function rollEntryInit(nodeEntry)
	...
	if sClass == "charsheet" then
		local nInitResult = CombatManager2.rollRandomInit(nInit, bADV, bDIS);
